...
console.log(`
    ...
    비동기(Asynchronous): 작업을 예약하고 바로 다음 코드로 넘어감
...
// 3초 후에 실행
// setTimeout(() => {
//     console.group('[3] Mock API 호출 - 실행 결과'); // 그룹 보기 창이 펼쳐진 상태로 시작
//     console.log('서버에 정보 요청 중...');
//     fetchUser(1, (user) => {
//         console.log('✅ 사용자 정보 수신 완료: ');
//         console.table(user);    // 객체를 테이블 형식으로 출력
//         console.groupEnd();    
//     });
// }, 3000);
...
function checkPermission (userInfo, action, callback) {      // 특정 권한 보유 여부 확인
    setTimeout(() => {
...
setTimeout(() => {
    ...
    login('dev.lee','password123', (token) => {     // 1단계: 로그인 후 토큰 받기
        ...
        getUserInfo(token, (userInfo) =>{       // 2단계: 토큰으로 사용자 정보 조회 (로그인 완료 후)
