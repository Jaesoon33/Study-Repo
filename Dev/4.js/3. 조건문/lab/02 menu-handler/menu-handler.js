...
function daycalc(day){
    switch(day){
        case 0: console.log("일요일: 휴식"); break;
        case 1: console.log("월요일: 주간 회의"); break;
        case 2: console.log("화요일: 개발 집중"); break;        
        case 3: console.log("수요일: 코드 리뷰"); break;        
        case 4: console.log("목요일: 테스트"); break;        
        case 5: console.log("금요일: 배포 & 회고"); break;        
        case 6: console.log("토요일: 자기 계발"); break;        
        default: console.log("잘못된 요일")
    }
}
...
/*
    문제 2: 요일별 처리 (입력)
        - prompt()로 사용자 입력을 받아 테스트 // prompt로 받은건 값들은 String값으로 저장됨
...
/*
    문제 3: HTTP 상태코드 선택
        - 실무에서 자주 쓰이는 패턴
        - HTML <select> 드롭다운으로 사용자가 입력을 받아 처리
...
// DOM Tree에서 ID로 요소를 찾아 반환 => JS 변수로 선언
const selectElement = document.getElementById("statusCodeSelect");
console.log('123',selectElement);
...
function handleStatusCode(code) {
    ...
    switch(code){
        ...
        case 500:
            ...
            break;
        default: message = "상태 코드를 선택해주세요."
    }

    return message;
}

// selectElement "change" 이벤트 리스너 등록 함수 내 값들은 static
selectElement.addEventListener("change", function(){ // 익명 함수
    const statusCode = parseInt(selectElement.value); // 처음 statusCode의 값은 "--선택해주세요--", val = NaN
...
*/
/*
    객체(Object)로 여러 값을 한번에 반환
    
    함수에서는 return으로 값을 하나만 반환할 수 있음
    하지만 여러 값을 "묶어서" 객체로 만들면 한 번에 전달이 가능함

    (ES6 문법)
    {
        icon: icon
        statusMessage: statusMessang,
        nextAction:nextAction
    }

    위와 같이 객체의 변수명과 속성명이 같으면 축약이 가능함
